...
type FoundationEventInput =
  ...
  | { readonly type: "follow.changed"; readonly commandId: OnTheGoFoundationCommand["commandId"] }
  | {
...
export const dispatchOnTheGoFoundation = (
  ...
  switch (command.type) {
    ...
    case "follow.switch": {
      if (state.followedChatId !== command.expectedChatId) {
    ...
    case "follow.checkpoint.record": {
      ...
      const pending = [...state.followPendingCheckpoints, checkpoint];
      const immediate = ["approval", "blocked", "completed", "failed"].includes(checkpoint.kind);
      const rateLimitElapsed =
        state.followLastSummaryAt === null ||
        epoch(now) - epoch(state.followLastSummaryAt) >= 30_000;
      if (!immediate && !rateLimitElapsed) {
        return accepted(
          command,
          { ...state, followPendingCheckpoints: pending },
          { type: "follow.changed", commandId: command.commandId },
        );
      }
      ...
      return accepted(
        command,
        {
          ...state,
          followPendingCheckpoints: [],
          followLastSummaryAt: now,
          followQuietCueAt: null,
          followTimeline: [
            ...state.followTimeline,
            {
              timelineId: `follow:${first.checkpointId}:${last.checkpointId}`,
              ...
              createdAt: now,
              priority: immediate ? "immediate" : "ordinary",
            },
          ],
        },
        { type: "follow.changed", commandId: command.commandId },
      ...
      return accepted(
        command,
        {
          ...
          followTimeline: [
            ...state.followTimeline,
            {
              timelineId: `catch-up:${first.checkpointId}:${last.checkpointId}`,
    ...
    case "follow.quiet-tick": {
      if (
        ...
        (state.followQuietCueAt !== null &&
          epoch(state.followQuietCueAt) >= epoch(state.followLastSummaryAt))
      ) {
        return accepted(command, state);
      }
      const lastCheckpoint =
        state.followPendingCheckpoints.at(-1)?.checkpointId ??
        state.followTimeline.at(-1)?.toCheckpointId ??
        "follow-start";
      return accepted(
        command,
        {
          ...state,
          followQuietCueAt: now,
          followTimeline: [
            ...state.followTimeline,
            {
              timelineId: `quiet:${state.followedChatId}:${now}`,
              chatId: state.followedChatId,
              fromCheckpointId: lastCheckpoint,
              toCheckpointId: lastCheckpoint,
              summary: "Still following. There has not been a new material checkpoint.",
              evidence: [],
              createdAt: now,
              priority: "quiet",
            },
    ...
    case "profile.observe": {
      if (
        command.sensitive ||
        command.oneOff ||
        command.confidence === "uncertain" ||
        /password|secret|token|api[_ -]?key/i.test(`${command.preference} ${command.evidence}`)
      ) {
    ...
    case "delivery.resolve": {
      ...
      if (!submission) return fail("not-found");
      if (terminalPendingStates.has(submission.state)) return accepted(command, state);
      const nextState: OnTheGoPendingTurn["state"] =
        command.outcome === "dispatched"
          ? "dispatched"
          : command.outcome === "unknown"
            ? "unknown-outcome"
            : "frozen";
      const effectId = `continue:${submission.submissionId}`;
      const effectStatus =
        command.outcome === "dispatched"
          ? "completed"
          : command.outcome === "unknown"
            ...
            pendingTurns: state.pendingTurns.map((turn) =>
              turn.submissionId === submission.submissionId
                ? {
                    ...turn,
                    state: nextState,
                    terminalAt: terminalPendingStates.has(nextState) ? now : null,
                  }
                : turn,
            ),
            frozenAgents:
              command.outcome === "failed"
                ? [...new Set([...state.frozenAgents, submission.targetAgentId])]
                : state.frozenAgents,
          },
          effectId,
          "turn-delivery",
