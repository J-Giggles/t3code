...
export const agentRunningTerminalIdsAtom = Atom.family((filter: KnownTerminalSessionListFilter) =>
  Atom.make((get) => {
    return pipe(
      Object.values(get(terminalSessionMetadataAtom(filter.environmentId))),
      Arr.filterMap((entry) =>
        ...
        (filter.threadId === null || entry.target.threadId === filter.threadId) &&
        (filter.terminalId === null || entry.target.terminalId === filter.terminalId) &&
        entry.summary.owner.kind === "agent" &&
        entry.summary.backing === "pty" &&
        (entry.summary.status === "starting" || entry.summary.status === "running")
          ? Result.succeed(entry.target.terminalId)
          : Result.failVoid,
      ),
      Arr.sort(Order.String),
    ...
    Atom.keepAlive,
    Atom.withLabel(`terminal-session:agent-running-terminal-ids:${JSON.stringify(filter)}`),
  ),
);
