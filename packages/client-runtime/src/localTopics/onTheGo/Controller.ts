...
export interface OnTheGoSpeechAdapter {
  readonly availability: () => { readonly available: boolean; readonly background: boolean };
...
export const makeOnTheGoController = (options: OnTheGoControllerOptions): OnTheGoController => {
  ...
  const speak = async (text: string) => {
    caption = text;
    notify();
    await options.speech.speak(text);
  };
  ...
  const preparePrompt = async (content: string) => {
    ...
    stagedHandoff = null;
    await speak(`Prepared a queued prompt for ${target.targetAgentId}. Say Send it to submit it.`);
    return true;
  };
  ...
  const acceptTranscript = async (
    ...
    if (commandPhrase === "what was the last announcement") {
      const result = await dispatch({
        ...
        commandId: commandId("response-last"),
        deviceId: options.scope.deviceId,
        direction: "last",
      });
      if (result.status !== "accepted") {
        await speak("There is no announcement to read");
        return;
      }
      await refresh();
      const response = focusedResponse();
      await speak(response?.safeSummary ?? "There is no announcement to read");
      return;
    }
    if (localMode === "theo-conversation") {
      theoMessages = [...theoMessages, { role: "user", text: transcript }];
      ...
      activeTheo = null;
      theoMessages = [...theoMessages, { role: "theo", text: result.reply }];
  ...
  return {
        ...
        stopEvents = options.transport.subscribe(options.scope, () => {
          ...
          void refresh()
            .then(() => {
              ...
              if (nextAttentionBadge > previousAttentionBadge) options.speech.tone?.("attention");
              else if (nextResponseBadge > previousResponseBadge) options.speech.tone?.("response");
              ...
              if (followTimeline.length > previousFollowCount) {
                ...
                if (!latest) return;
                if (localMode === "theo-conversation" || localMode === "dictation") {
    ...
    toggle: async (enabled) => {
      ...
      if (!availability.available) {
        caption = "Voice input is unavailable on this device";
        notify();
        return;
      }
