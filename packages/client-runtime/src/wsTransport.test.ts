...
describe("WsTransport", () => {
  ...
  it("appends websocket paths under path-prefixed base urls", async () => {
    const transport = createTransport("wss://app.example.com/t3code?token=secret-token");

    await waitFor(() => {
      expect(sockets).toHaveLength(1);
    });

    expect(getSocket().url).toBe("wss://app.example.com/t3code/ws?token=secret-token");
    await transport.dispose();
  });
  ...
  it("tracks heartbeat freshness from websocket pongs", async () => {
    ...
    expect(transport.getLastHeartbeatPongAt()).toBeNull();
    ...
    expect(transport.getLastHeartbeatPongAt()).toBe(1_000);
