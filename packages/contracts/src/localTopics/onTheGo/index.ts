...
export const OnTheGoFallbackModels = Schema.Struct({
  transcription: Schema.Array(OnTheGoTranscriptionModelSelection).pipe(
    Schema.withDecodingDefault(Effect.succeed([])),
  ),
  reasoning: Schema.Array(OnTheGoTheoModelSelection).pipe(
    Schema.withDecodingDefault(Effect.succeed([])),
  ),
  speech: Schema.Array(OnTheGoSpeechModelSelection).pipe(
    Schema.withDecodingDefault(Effect.succeed([])),
  ),
});
export type OnTheGoFallbackModels = typeof OnTheGoFallbackModels.Type;

export const OnTheGoRemoteCallBudget = Schema.Struct({
  warningAt: PositiveInt.pipe(Schema.withDecodingDefault(Effect.succeed(80))),
  hardLimit: PositiveInt.pipe(Schema.withDecodingDefault(Effect.succeed(100))),
});
export type OnTheGoRemoteCallBudget = typeof OnTheGoRemoteCallBudget.Type;
...
export const OnTheGoSettings = Schema.Struct({
  ...
  fallbackModels: OnTheGoFallbackModels.pipe(
    Schema.withDecodingDefault(Effect.succeed({ transcription: [], reasoning: [], speech: [] })),
  ),
  remoteCallBudget: OnTheGoRemoteCallBudget.pipe(
    Schema.withDecodingDefault(Effect.succeed({ warningAt: 80, hardLimit: 100 })),
  ),
});
...
export const DEFAULT_ON_THE_GO_SETTINGS: OnTheGoSettings = {
  ...
  fallbackModels: { transcription: [], reasoning: [], speech: [] },
  remoteCallBudget: { warningAt: 80, hardLimit: 100 },
};
