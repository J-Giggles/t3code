...
// @effect-diagnostics nodeBuiltinImport:off globalConsole:off globalDate:off - Server-local ops orchestration for scheduled git replay and messaging.
...
function collectConflictArtifacts(artifactDir: string): ReadonlyArray<string> {
  const fixedArtifacts = [
    ...
    "conflict-brief.md",
    "conflict-brief.telegram.html",
    "conflict-decision-card.telegram.html",
    "topic-stack-checklist.telegram.html",
...
export function formatTopicCatalog(input: NightlyAgentReportInput): string {
  const lines: Array<string> = [
    ...
    "",
    "This catalog is generated by the nightly upstream agent so the notification chat can answer questions about the replayed local topic stack.",
    "",
    "## How To Ask The Chat",
    "",
    `- Ask: \`Jordan's Hermes, read ${input.topicCatalogPath} and summarize the remote-access topic.\``,
    `- Ask: \`Jordan's Hermes, read ${input.topicCatalogPath} and list the tests for the provider-settings topic.\``,
    `- Ask: \`Jordan's Hermes, read ${input.reportPath} and tell me whether the nightly replay had conflicts.\``,
    "",
    "## Run Context",
    "",
    `- Status: \`${input.status}\``,
...
interface TelegramTargetParts {
  readonly chatId: string;
  readonly threadId?: string;
}

function parseTelegramTarget(target: string): TelegramTargetParts | undefined {
  if (!target.startsWith("telegram:")) return undefined;
  const [, chatId, threadId] = target.split(":");
  if (chatId === undefined || chatId.length === 0) return undefined;
  return {
    chatId,
...
export function formatConflictBriefGenerationPrompt(input: {
  ...
  return [
    ...
    "",
    "Write a Telegram-ready Conflict Brief for Jordan. Do not edit files. Do not continue the cherry-pick.",
    "",
    "The brief must be feature-level, not file-by-file, and must fit comfortably in one Telegram message.",
    ...
    "End with: Tap an option in the Conflict Decision Card.",
  ].join("\n");
}
...
function parseArgs(args: ReadonlyArray<string>): ParsedNightlyAgentArgs {
  ...
  let notify = parseBooleanEnv("T3CODE_NIGHTLY_NOTIFY", true);
  ...
  let hermesTarget = process.env.T3CODE_NIGHTLY_HERMES_TARGET ?? "telegram";
  let help = false;
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === "--help" || arg === "-h") {
      ...
      notify = true;
    } else if (arg === "--hermes-target") {
      hermesTarget = args[++index] ?? hermesTarget;
    } else if (arg.startsWith("--hermes-target=")) {
      hermesTarget = arg.slice("--hermes-target=".length);
...
function helpText(): string {
  return [
    "Usage: pnpm run nightly:upstream-agent -- [--root <repo-root>] [--force] [--no-notify]",
    "",
    "Fetches upstream/main, rebuilds .worktrees/nightly only when upstream changed, writes a",
    "deterministic report, and sends a Hermes notification.",
    "",
    "Environment:",
    "  T3CODE_NIGHTLY_HERMES_TARGET=telegram:-1001234567890",
...
function writeTelegramHtmlMessages(
  path: string,
  messages: ReadonlyArray<TelegramHtmlMessage>,
): void {
  writeText(
    path,
    messages
      .map((message, index) => [`<!-- message ${index + 1} -->`, message.html].join("\n"))
      .join("\n\n"),
  );
}

function telegramHtmlToPlain(html: string): string {
  return html
    .replace(/<br\s*\/?>/giu, "\n")
    .replace(/<\/(b|code)>/giu, "")
    .replace(/<(b|code)>/giu, "")
...
function telegramHtmlMessagesToPlain(messages: ReadonlyArray<TelegramHtmlMessage>): string {
  return messages.map((message) => telegramHtmlToPlain(message.html)).join("\n\n---\n\n");
}

function sendHermesNotification(
  target: string,
  subject: string,
  bodyPath: string,
  cwd: string,
  ...
  return runCommand(
    "hermes",
    ["send", "--to", target, "--subject", subject, "--file", bodyPath],
    cwd,
    {
      allowFailure: true,
    },
...
function readHermesTelegramBotToken(): string | undefined {
  const home = process.env.HOME;
  if (home === undefined) return undefined;
  const envPath = NodePath.join(home, ".hermes/.env");
  if (!NodeFS.existsSync(envPath)) return undefined;
  for (const line of NodeFS.readFileSync(envPath, "utf8").split(/\r?\n/u)) {
    if (line.trim().length === 0 || line.trimStart().startsWith("#") || !line.includes("=")) {
      continue;
    }
    const [key, ...valueParts] = line.split("=");
    if (key?.trim() !== "TELEGRAM_BOT_TOKEN") continue;
    return valueParts
      .join("=")
      .trim()
      .replace(/^['"]|['"]$/gu, "");
...
function sendTelegramHtmlMessage(input: {
  readonly target: TelegramTargetParts;
  ...
  const payload: Record<string, unknown> = {
    ...
    ...(input.target.threadId === undefined
      ? {}
      : { message_thread_id: Number(input.target.threadId) }),
...
try:
    ...
    if "reply_markup" in payload:
        ...
        send(payload)
        sys.stderr.write("sent without reply_markup fallback\n")
    else:
        sys.stderr.write(detail)
        sys.exit(1)
`;
  const result = runProcessCommand("python3", ["-c", script], input.cwd, {
    allowFailure: true,
    env: {
  ...
  return {
    ...result,
    command: `telegram.sendMessage --html --to telegram:${input.target.chatId}${
      input.target.threadId === undefined ? "" : `:${input.target.threadId}`
    }`,
  };
}

function sendTelegramHtmlMessagesWithFallback(input: {
  readonly target: string;
  readonly messages: ReadonlyArray<TelegramHtmlMessage>;
  readonly fallbackSubject: string;
  readonly fallbackPath: string;
  readonly cwd: string;
}): ReadonlyArray<CommandResult> {
  const telegramTarget = parseTelegramTarget(input.target);
  const token = readHermesTelegramBotToken();
  if (telegramTarget !== undefined && token !== undefined) {
    const directResults = input.messages.map((message) =>
    ...
    if (directResults.every((result) => result.exitCode === 0)) {
      return directResults;
    }
    return [
      ...directResults,
      sendHermesNotification(input.target, input.fallbackSubject, input.fallbackPath, input.cwd),
    ];
  }
...
function sendUpstreamSummaryNotification(input: {
  readonly target: string;
  readonly artifactDir: string;
  readonly notice: NightlyUpstreamNoticeInput;
  readonly cwd: string;
}): ReadonlyArray<CommandResult> {
  const messages = formatUpstreamTelegramNoticeMessages(input.notice);
  const htmlPath = NodePath.join(input.artifactDir, "telegram-upstream-summary.telegram.html");
  const fallbackPath = NodePath.join(input.artifactDir, "telegram-upstream-summary.md");
  writeTelegramHtmlMessages(htmlPath, messages);
  writeText(fallbackPath, telegramHtmlMessagesToPlain(messages));
  return sendTelegramHtmlMessagesWithFallback({
    target: input.target,
    messages,
    fallbackSubject: "[T3 Code Nightly] upstream summary",
    fallbackPath,
    cwd: input.cwd,
  });
}

function sendConflictBriefNotification(input: {
  readonly target: string;
  readonly artifactDir: string;
  readonly conflictBriefPath: string;
  readonly reportPath: string;
  ...
  const markdown = NodeFS.readFileSync(input.conflictBriefPath, "utf8");
  const messages = formatConflictBriefTelegramMessages({
    markdown,
    reportPath: input.reportPath,
    topicCatalogPath: input.topicCatalogPath,
  });
  const decisionCard = formatConflictDecisionCardTelegramMessage({
    markdown,
    conflictPromptPath: NodePath.join(input.artifactDir, "hermes-conflict-prompt.md"),
    reportPath: input.reportPath,
    topicCatalogPath: input.topicCatalogPath,
    runId: NodePath.basename(input.artifactDir),
    conflictTopicId: readFirstConflictRecord(input.artifactDir)?.id ?? "current-topic",
  });
  writeTelegramHtmlMessages(
    NodePath.join(input.artifactDir, "conflict-brief.telegram.html"),
    messages,
  );
  writeTelegramHtmlMessages(
    NodePath.join(input.artifactDir, "conflict-decision-card.telegram.html"),
    [decisionCard],
  );

  return sendTelegramHtmlMessagesWithFallback({
    target: input.target,
    messages: [...messages, decisionCard],
    fallbackSubject: "[T3 Code Nightly] Conflict Brief",
    fallbackPath: input.conflictBriefPath,
    cwd: input.cwd,
  });
}

function sendTopicStackChecklistNotification(input: {
  readonly target: string;
  readonly report: NightlyAgentReportInput;
  readonly cwd: string;
}): ReadonlyArray<CommandResult> {
  const messages = formatTopicStackChecklistTelegramMessages({
    status: input.report.status,
    decisionReason: input.report.decisionReason,
    ...(input.report.upstreamBefore === undefined
      ? {}
      : { upstreamBefore: input.report.upstreamBefore }),
    ...(input.report.upstreamAfter === undefined
      ? {}
      : { upstreamAfter: input.report.upstreamAfter }),
    topicSummaries: input.report.topicSummaries,
    reportPath: input.report.reportPath,
    topicCatalogPath: input.report.topicCatalogPath,
  });
  const htmlPath = NodePath.join(input.report.artifactDir, "topic-stack-checklist.telegram.html");
  const fallbackPath = NodePath.join(
    input.report.artifactDir,
    "topic-stack-checklist.telegram.txt",
  );
  writeTelegramHtmlMessages(htmlPath, messages);
  writeText(fallbackPath, telegramHtmlMessagesToPlain(messages));
  return sendTelegramHtmlMessagesWithFallback({
    target: input.target,
    messages,
    fallbackSubject: "[T3 Code Nightly] Topic Stack Checklist",
    fallbackPath,
    cwd: input.cwd,
  });
}

function sendAppliedTopicsNotification(input: {
  readonly target: string;
  readonly report: NightlyAgentReportInput;
  readonly cwd: string;
}): ReadonlyArray<CommandResult> {
  ...
  const htmlPath = NodePath.join(input.report.artifactDir, "applied-topics.telegram.html");
  const fallbackPath = NodePath.join(input.report.artifactDir, "applied-topics.telegram.txt");
  writeTelegramHtmlMessages(htmlPath, messages);
  writeText(fallbackPath, telegramHtmlMessagesToPlain(messages));
  return sendTelegramHtmlMessagesWithFallback({
    target: input.target,
    messages,
    fallbackSubject: "[T3 Code Nightly] Applied Topics",
    fallbackPath,
    cwd: input.cwd,
  });
}

function sendFinalSummaryNotification(input: {
  readonly target: string;
  readonly report: NightlyAgentReportInput;
  readonly fallbackPath: string;
  readonly cwd: string;
}): ReadonlyArray<CommandResult> {
  const messages = formatFinalTelegramSummaryMessages(input.report);
  writeTelegramHtmlMessages(
    NodePath.join(input.report.artifactDir, "telegram-summary.telegram.html"),
    messages,
  );
  return sendTelegramHtmlMessagesWithFallback({
    target: input.target,
    messages,
    fallbackSubject: `[T3 Code Nightly] ${input.report.status}`,
...
function generateHermesConflictBrief(input: {
  readonly artifactDir: string;
  readonly reportPath: string;
  readonly topicCatalogPath: string;
  readonly hermesTarget: string;
  ...
  writeText(conflictBriefPath, normalizeConflictBriefForTelegram(generateResult.stdout.trim()));
  const sendResults = sendConflictBriefNotification({
    target: input.hermesTarget,
  ...
  commandResults.push(...sendResults);
  const finalDeliveryResult = sendResults.at(-1);
  if (finalDeliveryResult === undefined || finalDeliveryResult.exitCode !== 0) {
    const error = `delivery failed: ${
      finalDeliveryResult === undefined
        ? "no delivery command ran"
        : firstUsefulError(finalDeliveryResult)
    }`;
    writeText(conflictBriefErrorPath, `${error}\n`);
    return { commandResults, conflictBriefPath, conflictBriefError: error };
  }
...
function main(): void {
  ...
  if (args.notify) {
    const runningPath = NodePath.join(fallbackArtifactDir, "telegram-running.md");
    writeText(
      runningPath,
    ...
    commandResults.push(
      sendHermesNotification(
        args.hermesTarget,
        "[T3 Code Nightly] running",
        runningPath,
        controlRoot,
      ),
    );
  ...
  try {
    ...
    const pausedArtifactDir = nightlyInterrupted ? latestRunDir(nightlyPath) : undefined;
    ...
    if (args.notify) {
      commandResults.push(
        ...sendUpstreamSummaryNotification({
          target: args.hermesTarget,
          artifactDir: fallbackArtifactDir,
          notice: {
            decisionReason,
            willReplay: decision.apply,
            ...(decision.blocker === undefined ? {} : { blocker: decision.blocker }),
            ...(upstreamBefore === undefined ? {} : { upstreamBefore }),
            ...(upstreamAfter === undefined ? {} : { upstreamAfter }),
  ...
  const telegramPath = NodePath.join(artifactDir, "telegram-summary.md");
  ...
  writeText(telegramPath, formatTelegramSummary(reportInput));

  if (args.notify && status === "failed") {
    const conflictBrief = generateHermesConflictBrief({
      artifactDir,
      reportPath,
      topicCatalogPath,
      hermesTarget: args.hermesTarget,
      cwd: controlRoot,
    });
  ...
  if (args.notify) {
    const appliedTopicsResults = sendAppliedTopicsNotification({
      target: args.hermesTarget,
      report: buildReportInput(),
      cwd: controlRoot,
    });
    commandResults.push(...appliedTopicsResults);
    reportInput = buildReportInput();
    writeText(reportPath, formatNightlyAgentMarkdown(reportInput));
    writeText(topicCatalogPath, formatTopicCatalog(reportInput));
    writeText(telegramPath, formatTelegramSummary(reportInput));

    const checklistResults = sendTopicStackChecklistNotification({
      target: args.hermesTarget,
      report: buildReportInput(),
      cwd: controlRoot,
    });
    commandResults.push(...checklistResults);
    reportInput = buildReportInput();
    writeText(reportPath, formatNightlyAgentMarkdown(reportInput));
    writeText(topicCatalogPath, formatTopicCatalog(reportInput));
    writeText(telegramPath, formatTelegramSummary(reportInput));
  ...
  if (args.notify) {
    const notifyResults = sendFinalSummaryNotification({
      target: args.hermesTarget,
      report: buildReportInput(),
      fallbackPath: telegramPath,
      cwd: controlRoot,
    });
    commandResults.push(...notifyResults);
    const finalResult = notifyResults.at(-1);
    if (finalResult !== undefined && finalResult.exitCode !== 0 && status !== "failed") {
      status = "failed";
      errorMessage = finalResult.stderr || finalResult.stdout || "Final notification failed";
    }
  }
...
export function runNightlyUpstreamAgentCli(): void {
  try {
    main();
  } catch (error) {
