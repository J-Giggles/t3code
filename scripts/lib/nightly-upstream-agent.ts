...
// @effect-diagnostics nodeBuiltinImport:off globalConsole:off globalDate:off - Server-local ops orchestration for scheduled git replay and artifact reporting.
...
export function formatTopicCatalog(input: NightlyAgentReportInput): string {
  const lines: Array<string> = [
    ...
    "",
    "This catalog is generated by the nightly upstream agent so future agents can answer questions about the replayed local topic stack from durable local evidence.",
    "",
    ...
    "- The full report and run artifacts contain the topic checklist, proof, and promotion readiness.",
    "",
    "## Run Context",
    "",
    `- Status: \`${input.status}\``,
...
export function formatConflictBriefGenerationPrompt(input: {
  ...
  return [
    ...
    "",
    "Write a standalone Conflict Brief for Jordan. Do not edit files. Do not continue the cherry-pick.",
    "",
    "The brief must be feature-level, not file-by-file, and concise enough to scan in the run report.",
...
function helpText(): string {
  return [
    "Usage: pnpm run nightly:upstream-agent -- [--root <repo-root>] [--force]",
    "",
    "Fetches upstream/main, rebuilds .worktrees/nightly only when upstream changed, and writes",
    "deterministic local reports and proof artifacts.",
    "",
    "Environment:",
...
function generateConflictBrief(input: {
  readonly artifactDir: string;
  readonly reportPath: string;
  readonly topicCatalogPath: string;
  readonly cwd: string;
...
export function formatAutonomousRepairPrompt(input: {
  ...
  readonly resultPath: string;
  readonly repairEvidencePath: string;
  readonly conflictFiles: ReadonlyArray<string>;
}): string {
  ...
  return [
    ...
    "",
    "Execution context:",
    `- This scheduled operation records its host, worktree policy, service, timer, checks, and evidence contract in the run artifacts.`,
    `- This is a non-interactive worker. Do not create tracker issues or unrelated repo files. The parent records this attempt at ${input.repairEvidencePath}.`,
...
function runAutonomousRepairAttempt(input: {
  ...
  const repairEvidencePath = NodePath.join(
    input.artifactDir,
    `repair-evidence-attempt-${input.attempt}.md`,
  );
...
async function main(): Promise<void> {
  ...
  if (status === "failed") {
    const conflictBrief = generateConflictBrief({
      artifactDir,
      reportPath,
      topicCatalogPath,
      cwd: controlRoot,
