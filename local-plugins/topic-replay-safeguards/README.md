...
- [x] Routine replay conflicts are repaired autonomously inside declared topic paths while fundamental product decisions fail closed with a local Conflict Brief (`scripts/lib/nightly-upstream-agent.ts`, `docs/operations/nightly-upstream-agent.md`).
...
- [x] Actionable nightly runs record official changes, a topic checklist, repairs, proof, recommendations, and promotion readiness in durable local artifacts (`scripts/lib/nightly-upstream-agent.ts`, `docs/operations/nightly-upstream-agent.md`).
...
- [x] Every run writes a structured local report; successful candidates become promotion-eligible, fundamental decisions receive a Conflict Brief, and repairable failures remain failed (`scripts/lib/nightly-upstream-agent.ts`, `docs/operations/nightly-upstream-agent.md`).
- [x] Each run writes an agent-readable topic catalog so Codex can answer feature and test questions from durable server evidence (`scripts/lib/nightly-upstream-agent.ts`, `docs/operations/nightly-upstream-agent.md`).
...
- [x] Nightly agent tests cover artifact reporting, strict resume guards, repair classification, path restrictions, completed-stack proof, and applied-topic reporting (`scripts/nightly-upstream-agent.test.ts`).
...
Replay after the functional topic stack so README checklist conversion sees the current topic folders. If this topic conflicts, keep validator/checklist documentation changes together, keep audit and artifact reporting behavior with the nightly agent, and preserve exact-match repair memory, control-plane sync, and completed-stack verification as fail-closed safeguards. Dependency-floor repairs may update `apps/desktop/package.json` together with `pnpm-lock.yaml` when that is required to keep a newer upstream desktop dependency.
