...
| `OTG-UT-021` | Platform capability policy                         | Electron/mobile allow their declared background states; macOS waits for native microphone consent under its signed app identity; web loses voice availability when backgrounded; unsupported actions fail closed. |
...
| `OTG-UT-024` | Desktop authentication rotation                    | A relaunched desktop migrates a legacy session binding to its stable authenticated principal; later sessions for that principal reconnect normally; active sessions and different principals remain isolated.     |
...
- The macOS `OTG-UT-020/021` launch boundary is additionally pinned by `apps/desktop/scripts/electron-launcher.test.mjs`, `apps/desktop/scripts/dev-process-cleanup.test.mjs`, and `apps/desktop/src/window/DesktopWindow.test.ts`.
