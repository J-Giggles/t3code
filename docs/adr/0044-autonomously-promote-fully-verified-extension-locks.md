...
Current workspace policy requires explicit Main approval. Proposed ADR 0064 creates one deliberate exception when accepted: only the official T3 Extension Catalog coordinator may use the autonomous rule below, and only for an exact contract-preserving assembly with complete durable Nightly/Staging evidence. All other Main paths remain approval-gated.
